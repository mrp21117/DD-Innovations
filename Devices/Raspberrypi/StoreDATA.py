...
try:
    ...
    TM = time.strftime("%M")
    #Create logfile
    with open("Logs/logfile{TM}.txt",'w') as file:
        file.write(f"Connected to {converter_ip} on port {converter_port} \n \n")
        file.close()
        event = 1
        while True:
            #Read Inputs
            ...
            while(k):
                ...
                with open(f"Logs/logfile{TM}.txt",'a') as log:
                #Text File Store DATA logs
                    full_datetime = time.strftime("%d/%m/%y at %I:%M%p")
                    log.write(f"Event{event} :  {full_datetime} \n")
                    log.write(f"Inputs :  {PB_List} \n")
                    log.write(f"Outputs:  {coil_states} \n \n")
                    log.close()
                    event += 1
